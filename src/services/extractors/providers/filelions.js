...
export function match(url) {
    return url.includes('filelions') || url.includes('moonplayer');
}

export async function extract(embedUrl, req) {
    try {
        console.log(`[Filelions] Extracting (Axios): ${embedUrl}`);
        let fetchUrl = embedUrl;
        
        // Normalize /f/ to /e/ if needed
        if (embedUrl.match(/\/f\/[^/]+\/?$/)) {
            fetchUrl = embedUrl.replace(/\/f\//, '/e/');
            console.log(`[Filelions] Normalisasi URL: ${embedUrl} → ${fetchUrl}`);
        }
        ...
        if (packerMatch) {
            try {
                ...
                console.log('[Filelions] Unpack error:', e.message);
            }
        }
        ...
        if (videoMatch && videoMatch[1]) {
            ...
            console.info(`[Filelions] Found direct URL: ${videoUrl}`);
        ...
        console.log(`[Filelions] Extraction failed: ${e.message}, falling back to generic/Puppeteer`);
        return null;
    }
}
